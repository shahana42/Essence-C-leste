...
const ProductImage = ({ image, isEven }) => {
    ...
    return (
      <motion.div
        ...
        viewport={{ once: false }}
        whileHover={{ scale: 1.1, rotate: 3 }}  // Increased scale and rotate
  ...
  const ProductCard = ({ description }) => (
    <motion.div
      ...
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, delay: 0.2 }}  // Increased transition duration
      viewport={{ once: false }}
      whileHover={{ scale: 1.05 }}  // Slightly more scale effect on hover
    >
      <Card
        style={{
          maxWidth: "400px",
